...
// $Id: calendar.js,v 1.1.1.1 2007/01/27 04:14:35 liujuan Exp $
...
function insertCalendar(){
    includeCss();
    var cals = document.all('calendar');
    if (cals == null)return;
    if (cals.length == null)cals = [cals];
    for(var i = 0; i < cals.length; i++){
        cals[i].insertAdjacentHTML('afterEnd','<img src="'+ cal_rootpath + 'pop.gif" style="cursor: pointer; height:21" title="时间选择器" onmouseover="this.style.background=\'red\';"  onmouseout="this.style.background=\'\'" onclick="showcalendar(this.previousSibling)">');
        cals[i].onblur = function (){
            if(!(this.value.match(/^\s+$/))
                &&this.value!=''
                &&!(this.value.match(/^(\d{4})(-)((0[1-9])|(1[0-2]))(-)((0[1-9])|((1|2)[0-9])|(3(0|1)))$/))
                &&!(this.value.match(/^(\d{4})((0[1-9])|(1[0-2]))((0[1-9])|((1|2)[0-9])|(3(0|1)))$/))){
                alert('日期格式必须是YYYY-MM-DD或YYYMMDD');
                this.value='';this.select();
             }else if (this.value.match(/^(\d{4})((0[1-9])|(1[0-2]))((0[1-9])|((1|2)[0-9])|(3(0|1)))$/)){
